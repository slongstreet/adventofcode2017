...
func main() {
	testMap := buildTowerMap(loadTestData())
	for k, v := range testMap {
		if v.parent == nil {
			fmt.Printf("test root is %s\n", k)
			fmt.Printf("test expected weight: %v\n\n", findExpectedWeight(v))
			break
		}
	}

	towerMap := buildTowerMap(loadData())
	for k, v := range towerMap {
		if v.parent == nil {
			fmt.Printf("root is %s\n", k)
			fmt.Printf("expected weight: %v\n", findExpectedWeight(v))
			break
		}
	}
}
...
func buildTowerMap(input string) map[string]*Tower {
	...
	for _, line := range strings.Split(input, "\n") {
		...
		towerParts := strings.Split(parts[0], " (")
		var name = towerParts[0]
		var weight, _ = strconv.Atoi(strings.Split(towerParts[1], ")")[0])

		// get or add tower to map
		var tower *Tower
		if node, ok := towerMap[name]; ok {
			tower = node
		} else {
			tower = makeTowerNode(name, weight)
			towerMap[name] = tower
		}
		...
		if len(parts) > 1 {
			for _, child := range strings.Split(parts[1], ", ") {
				if _, ok := towerMap[child]; !ok {
					towerMap[child] = makeTowerNode(child, 0)
				}

				towerMap[child].parent = tower
				towerMap[child].parent.children = append(towerMap[child].parent.children, towerMap[child])
			}
		}
	}
...
func makeTowerNode(name string, weight int) *Tower {
	var tower = new(Tower)
	tower.name = name
	tower.weight = weight
	tower.children = make([]*Tower, 0)

	return tower
}

func findExpectedWeight(root *Tower) int {
	uniqueNode, uniqueWeight, otherWeights := getUniqueChildNode(root)
	var delta = uniqueWeight - otherWeights
	var unbalancedWeight = 0

	for {
		unbalancedWeight = uniqueNode.weight
		childNode, _, _ := getUniqueChildNode(uniqueNode)
		if childNode == nil {
			return unbalancedWeight - delta
		}

		uniqueNode = childNode
	}

	return 0
}

func getUniqueChildNode(root *Tower) (uniqueNode *Tower, uniqueWeight int, otherWeights int) {
	var childWeightMap = getChildWeightMap(root)
	debugPrintWeightMap(childWeightMap)

	var freqMap = make(map[int]int)
	var candidates = make(map[*Tower]int)

	for node, weight := range childWeightMap {
		freqMap[weight]++
		if freqMap[weight] == 1 {
			candidates[node] = weight
		} else {
			otherWeights = weight
		}
	}

	for node, weight := range candidates {
	...
	return uniqueNode, uniqueWeight, otherWeights
}

func getChildWeightMap(node *Tower) map[*Tower]int {
	var weightMap = make(map[*Tower]int)
	for _, child := range node.children {
		var total = getWeights(child)
		weightMap[child] = total
	}
...
func debugPrintWeightMap(weightMap map[*Tower]int) {
	var debugString = ""
	for node, weight := range weightMap {
		if debugString != "" {
			debugString += ", "
		}

		debugString += "(" + node.name + ": " + strconv.Itoa(weight) + ")"
	}
